      ...
      <div className="flex-grow flex flex-col min-w-0 h-full bg-white rounded-none md:rounded-[28px] border-0 md:border md:border-slate-200/70 md:shadow-[0_4px_30px_rgba(0,0,0,0.03)] overflow-hidden relative">
        ...
        <header className="px-3.5 sm:px-6 pt-[max(6px,calc(env(safe-area-inset-top,0px)+2px))] pb-2 sm:py-3.5 flex items-center justify-between border-b border-slate-100/90 select-none shrink-0 bg-white backdrop-blur-md sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3 min-w-0 relative">
            {/* Mobile: Modern User Avatar & Identity Chip */}
            <div className="md:hidden flex items-center min-w-0">
              <button
                type="button"
                onClick={() => setProfileDropdownOpen(!profileDropdownOpen)}
                className="flex items-center gap-2.5 p-1 pr-2 rounded-2xl hover:bg-slate-50 active:scale-98 transition-all cursor-pointer group text-left min-w-0 border border-transparent hover:border-slate-200/60"
                title="Quản lý tài khoản & Cài đặt"
              >
                {/* Avatar with Status Ring */}
                <div className="relative shrink-0">
                  <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-orange-500/25 bg-white shadow-3xs group-hover:scale-105 transition-transform ring-2 ring-orange-100/80">
                    <img 
                      src={currentUser?.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop&crop=faces'} 
                      alt="Avatar" 
                      className="w-full h-full object-cover" 
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[8px] font-black uppercase px-1.5 py-0.2 rounded bg-orange-100 text-orange-700 tracking-wide shrink-0">
                      {currentUser?.role === 'admin' ? 'Admin' : 'Staff'}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 mt-0.5">
                    <span className="text-[10px] font-bold text-gray-400 truncate leading-tight">
                      {logoText || 'TIỆM ẢNH NHÀ CAOS'}
                    </span>
                    <ChevronDown className={`w-3 h-3 text-gray-400 group-hover:text-gray-700 transition-transform ${profileDropdownOpen ? 'rotate-180' : ''}`} />
                  </div>
                </div>
              </button>

              {/* Mobile Profile & System Dropdown */}
              {profileDropdownOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setProfileDropdownOpen(false)} />
                  <div className="absolute left-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200/90 py-1.5 z-50 text-left text-xs text-slate-800 animate-fade-in divide-y divide-slate-100 w-64 max-w-[calc(100vw-32px)]">
                    <div className="px-4 py-3 bg-gradient-to-br from-slate-50 to-orange-50/40">
                      <div className="flex items-center gap-2.5">
                        <div className="w-10 h-10 rounded-full overflow-hidden border border-orange-200 shrink-0">
                          <img 
                            src={currentUser?.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop&crop=faces'} 
                            alt="Avatar" 
                            className="w-full h-full object-cover" 
                    ...
                    <div className="py-1">
                      <button
                        ...
                        className="w-full px-3.5 py-2.5 hover:bg-rose-50 text-rose-600 font-bold text-left transition flex items-center gap-2.5 cursor-pointer"
                      >
          ...
          {/* Right: Clean Minimal Actions (Search, Date, Notification) */}
          <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
            ...
            <button
              ...
              className="w-10 h-10 sm:w-9 sm:h-9 rounded-xl border border-slate-200/80 bg-slate-50/60 hover:bg-slate-100 text-slate-500 hover:text-slate-900 flex items-center justify-center transition cursor-pointer shadow-3xs group active:scale-95"
          ...
          {[
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center flex-1 py-0.5 px-0.5 transition-all duration-150 cursor-pointer active:scale-92 ${
              ...
              >
                ...
                <span className={`text-[10px] tracking-tight mt-1 transition-all leading-none ${
                  isActive ? 'font-black text-orange-600' : 'font-semibold text-gray-500'
                }`}>
                  {tab.label}
                </span>
              </button>
